...
# Retrieval-only LifeBench eval — zero LLM cost.
# Usage:
#   bash run-retrieval-lifebench.sh [QUERY_LIMIT]
#
# Split is always "en". ~3,605 docs, 2,003 queries, 10 users.
# Estimated time: ~30 min per run.
...
echo "=== LifeBench (en) retrieval-only eval ==="
