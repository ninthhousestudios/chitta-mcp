...
|---|---|---|
| `tool` | yes | Which tool or subsystem produced the error (e.g., `store_memory`, `get_memory`, `update_memory`, `delete_memory`, `database`, `startup`, `server`) |
...
| `content` | <= 4 MB bytes | Reduce content size. Split into multiple memories if needed. |
...
| `content/tags` | at least one required (update_memory) | Provide content and/or tags to update. |

All `InvalidArgument` errors include `received` with the offending value so the caller can see exactly what was rejected.

### ContentTooLong

The content exceeds BGE-M3's 8192-token context window. This is checked after tokenization, so the count is exact (not an estimate). Applies to `store_memory`, `update_memory` (when content is provided), and `search_memories` (query text).
...
The requested `(profile, id)` does not match any row. Raised by `get_memory`, `update_memory`, and `delete_memory`.
...
An error in the embedding pipeline: tokenizer failure, ONNX session error, unexpected output shape, or session panic.
...
```json
{
  ...
  "data": {
    ...
    "received": { "message": "ONNX session panicked and was replaced" },
    "next_action": "Retry the request."
  }
}
```

At startup, embedding errors include more specific guidance (check `CHITTA_MODEL_PATH`, ensure `libonnxruntime` is installed). For session panics, the `next_action` is "Retry the request" because the session has been automatically replaced.
...
Catch-all for unexpected server faults (mutex poisoning, serialization failure, spawn_blocking failure, embedding timeout). The `next_action` is always "Report this as a bug; include server logs."
